...
## Quickstart: Supabase + Google Sheets deployment

1. Create a Supabase project and copy the URL and ANON key.
2. Create a Google Cloud API key for Google Sheets API and make the spreadsheet public (or use a serverless proxy).
3. Create a `.env` file in project root with:

```
VITE_SUPABASE_URL=https://your-project.supabase.co
VITE_SUPABASE_ANON_KEY=your-anon-key
VITE_SHEETS_SPREADSHEET_ID=your-spreadsheet-id
VITE_SHEETS_RANGE=Sheet1!A1:Z1000
VITE_SHEETS_API_KEY=your-google-api-key
```
...
5. Deploy on Vercel: push to GitHub and set the same environment variables in the Vercel project settings.

Security note: For production, consider proxying Google Sheets requests through a serverless function to keep the API key secret.

### Creating the single admin user (manual steps)

1. In Supabase dashboard, go to Authentication > Users and create a new user with the admin email and password:

  - Email: erikfabiano082@gmail.com
  - Password: chamados123

2. Open SQL Editor in Supabase and run the file `SUPABASE_ADMIN_SETUP.sql` included in this repository to insert the admin into `app_admins` table.

3. After running the SQL and creating the Auth user, o frontend aceitará apenas esse email como admin.
